...
这里，我们选择 oauth2.0 来实现这个功能

永远在线的用户
--------------
到目前为止，用户只要通过授权操作，就可以在我们的应用中访问到微博的内容了。
我们也知道，要从微博获取信息，必须以某个用户的身份发起请求。
那么，对于没有经过授权或登录操作的游客用户，应用该如何从微博获取信息并展示给他们呢？

看来，我们需要有一个用户随时待命，永远在线，专门为没有登录的用户获取数据。
...
实际上，我们将使用一个更加可靠的途径来实现这个永远在线的用户。

我们将手动授权一次，然后记录 Access Token，对微博的操作都可以通过这个 Access Token 来进行。

但是，目前无论是新浪还是腾讯，通过 oauth2.0 鉴权得到的 Access Token 都是有有效期的，不适用于我们的情况。

而 oauth1.0 鉴权得到的 Access Token 和 Access Token Secret 的有效期则是没有限制的，除非用户主动取消授权。
...
要完成这个过程，并不需要我们亲自去写这几个请求，腾讯微博提供了一个在线的授权及接口调试工具。
通过这个工具我们能方便地获取到 Access Token 和 Access Token Secret 。

首先把授权方式设为 OAuth1.0，然后在接口列表中选择 request\_token，按照要求填写参数，其中 oauth\_signature 的算法可以在 Oauth1.0 的文档中找到，但如果你想偷懒的话，可以随便填些内容，然后点击下方的“检查问题”，系统会通过提示告诉你正确的签名值。
把正确的签名值天道 oauth\_signature 后，再次点击“检查问题”，就能通过并得到未授权的 oauth\_token 和 oauth\_token\_secret，把它们记下来备用。

接下来在接口列表设为 authorize，这里只需要填写一个参数，就是前面一步里得到的那个 oauth\_token，点击“检查问题”，会给出一个“授权”按钮，点击授权按钮就进入授权页面了。
...
所有参数填写正确后，点击“检查问题”给出的那个字符串中的 oauth\_token 和 oauth\_token\_secret 就是我们想要的已经经过授权并且长期有效的 Access Token 和 Access Token Secret 了。

不用写一行代码，我们就完成了实现一个永远在线的用户的准备工作。
